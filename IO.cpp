	...
	while(!(validBlack&&validWhite)){
		...
		if (stringplayers.size() == 2){
			if(stringplayers[0] == 'H' || stringplayers[0] == 'F'|| stringplayers[0] == 'A'){
				...
				cout << "mauvaise entr̩e du joueur noir, veuillez recommencer" <<endl;
				validBlack=false;
				}
			if(stringplayers[1] == 'H' || stringplayers[1] == 'F'||stringplayers[1] == 'A'){
				...
				cout << "mauvaise entr̩e du joueur blanc, veuillez recommencer" <<endl;
				validWhite = false;
			}
		}
		else{
...
void IO::boardDisplay(Board & board){
	cout<<"       a b c d e f g h  " << endl; //Impression des num̩ros de ligne
	for(unsigned int line =1; line <= 8; line++){
		cout<< "     "<<(line)<< " "; // Impression des num̩ros de colonne
